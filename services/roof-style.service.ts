...
export class RoofStyleService {
  async listPublished() {
    const items = await prisma.roofStyle.findMany({
      ...
      include: { covering: { select: { code: true, name: true, unitPrice: true } } },
    });
  ...
  async listAll() {
    const items = await prisma.roofStyle.findMany({
      orderBy: [{ sort: 'asc' }, { code: 'asc' }],
      include: {
        covering: { select: { code: true, name: true, unitPrice: true } },
  ...
  async create(input: RoofStyleInput, actorId: string | null = null) {
    ...
    await this.assertCovering(data.coveringId);
    this.assertMansard(data.family, data.pitch, data.upperPitch);
  ...
  async update(id: string, input: Partial<RoofStyleInput>, actorId: string | null = null) {
    ...
    if (data.coveringId) await this.assertCovering(data.coveringId);

    this.assertMansard(
      data.family ?? existing.family,
      data.pitch ?? existing.pitch,
      data.upperPitch ?? existing.upperPitch,
    );
  ...
  async delete(id: string, actorId: string | null = null) {
    ...
    const used = await prisma.style.count({ where: { roofStyleId: id } });
  ...
  private assertMansard(family: string, pitch: number, upperPitch?: number | null) {
